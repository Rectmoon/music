<template>
  <div class="picker-render-view">
    ...
    <picker @select="handleSelect(0,arguments)" :selected-index="selectedIndex[0]"
            ref="picker0" :title="title[0]" cancelTxt="取消"
            confirmTxt="确认"></picker>
    ...
    <picker @select="handleSelect(1,arguments)" :data="data[1]" :selected-index="selectedIndex[1]"
            ref="picker1" :title="title[1]" cancelTxt="取消"
            confirmTxt="确认"></picker>
    ...
    <picker @select="handleSelect(2,arguments)" :data="data[2]" :selected-index="selectedIndex[2]"
            ref="picker2" :title="title[2]" cancelTxt="取消"
            confirmTxt="确认"></picker> 
    ...
    <city-picker @select="handleSelect(3,arguments)" :data="data[3]" :selected-index="selectedIndex[3]"
                 ref="picker3" :title="title[3]" cancelTxt="取消"
                 confirmTxt="确认"></city-picker>

 <div class="select" @click="showPicker(4)" ref="select4">{{ selectedText[4] }}</div>
      <gw-picker @select="handleSelect(4,arguments)" :data="data[4]" :selected-index="selectedIndex[4]"
                 ref="picker4" :title="title[4]" cancelTxt="取消"
                 confirmTxt="确认"></gw-picker>  
  </div>
</template>
...
import GwPicker from '@/components/gw-picker/gw-picker.vue'
import { provinceList, cityList, areaList } from '@/lib/data/areaData'
...
export default {
  ...
  data() {
    return {
      ...
      selectedIndex: [[0], [1, 0], [0, 1, 2], [0, 0, 0], [1, 2, 5]],
      selectedText: [1, 2, 3, 4, 5],
      title: ['青龙', '白虎', '朱雀', '玄武', '网关']
    }
  },
  methods: {
    showPicker(index) {
      ...
      setTimeout(() => {
        if (index < 3) {
          picker.scrollTo(0, 10, 300)
        } else {
          picker.$refs.picker.scrollTo(0, 100, 300)
        }
      }, 500)
    },
